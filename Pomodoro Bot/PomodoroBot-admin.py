...
def postRequest(endpoint, body ):
    ...
    response = requests.post(baseURL + endpoint, data=json.dumps(body), headers=headers)
    print(response.text)
...
async def start_counter(ctx):
    ...
    last_30_uses = getRequest("last_30_use")

    print(last_30_uses)

    if len(last_30_uses) == 30:
        requests.delete(baseURL + "last_30_use/1"  )

    currentTime = datetime.datetime.now()

    x = postRequest("last_30_use",
    { "user": getName(ctx),
      "userID": ctx.author.id,
      "date": f"{currentTime:%d.%m.%Y}",
      "time":  f"{currentTime:%H:%M}",
      })
...
async def yardım(ctx):
    helpText = getRequest("texts/1")["content"]
    await ctx.send( helpText )

@bot.command(name="pomodoro")
async def whatIsPomodoro(ctx):
    pomodoroText = getRequest("texts/2")["content"]
    await ctx.send( pomodoroText )
